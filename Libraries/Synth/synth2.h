...
//
//  Dzl/Illutron 2014
//  Modified to remove MIDI, pitch modulation, and envelope modulation functions, for pure tone generation
//  Removed all wave table except SINE. Removed functions to set wave table type
//
//*************************************************************************************
...
#define FS 40000.0                                  //-Sample rate (Doubled for better tone quality - Uses more processor time)
...
SIGNAL(TIMER1_COMPA_vect)
{
  ...
  if (!(((unsigned char*)&EPCW[divider])[1]&0x80))
	AMP[divider] = 255 - VOL;  //Bypass volume envelope generation so envelope tables can be removed
  else
    AMP[divider] = 0;
